...
class SpiderBeta(scrapy.Spider):
    ...
    start_urls = [
                "https://sekolah.data.kemdikbud.go.id/index.php/Cpetasebaran/index/",
                ]
    ...
    def parse(self, response):
        if response.status == 503:
            ...
            ul_kabupaten = response.css('body > div.container > div > div:nth-child(2) > div > div > div:nth-child(1) > div:nth-child(1) > div > #kode_kabupaten > option')            
            for li_element in ul_kabupaten:
                li_text_kabupaten = li_element.css('::text').get()
                li_value = li_element.css('::attr(value)').get()
                ...
                for li_element in ul_jenjang:
                    ...
                    links = f'https://sekolah.data.kemdikbud.go.id/index.php/Cpetasebaran/index/{li_value}/{li_text}'
                
                    yield scrapy.Request(links, callback=self.parse_detail, 
                    meta={'kabupaten': li_text_kabupaten, 'li_value' : li_value}
                                    )
    ...
    def parse_detail(self, response):
        ...
        data_list = re.findall(r'<li class="list-group-item text-muted">NPSN : (\d+)</li>'
                                r'.*?<a href="(.+?)" target="_blank">(.+?)</a></li>'
                                r'.*?<b>Alamat</b> : (.+?)</li>'
                                r'.*?L\.marker\(L\.latLng\(([\d.]+),([\d.]+)\)\)'
                                ,data_script, re.DOTALL)
